...
export default function GirlfriendDayPage() {
  ...
  // --- "Click this" ask-dialog with a runaway No button ---
  const [askOpen, setAskOpen] = useState(false);
  const [imageOpen, setImageOpen] = useState(false);
  const [noPos, setNoPos] = useState(null); // null = default centered spot
  ...
  const dodgeNoButton = () => {
    ...
    const left = 10 + Math.random() * 70; // vw
    setNoPos({ top: `${top}vh`, left: `${left}vw` });
    setNoDodges((n) => n + 1);
  };
  ...
  return (
    <div className="gd-root">
      <style>{`
        ...
        .gd-ask-btn{
          font-family:'Outfit',sans-serif; font-weight:500; font-size:0.85rem; letter-spacing:0.05em;
          color:var(--navy-deep); background:linear-gradient(135deg,#fff8e6,var(--gold-soft));
          border:none; border-radius:999px; padding:0.75rem 1.8rem; margin-bottom:1.8rem; cursor:pointer;
          box-shadow:0 10px 25px -8px rgba(232,200,116,0.55); transition:transform .3s ease, box-shadow .3s ease;
        }
        .gd-ask-btn:hover{transform:translateY(-2px) scale(1.03); box-shadow:0 14px 30px -8px rgba(232,200,116,0.7);}
        .gd-ask-btn:active{transform:translateY(0) scale(0.98);}

        .gd-modal-overlay{
          position:fixed; inset:0; z-index:50; background:rgba(6,8,20,0.72); backdrop-filter:blur(3px);
          display:flex; align-items:center; justify-content:center; padding:1.5rem;
          animation:gdFadeIn .3s ease;
        }
        @keyframes gdFadeIn{from{opacity:0;} to{opacity:1;}}
        .gd-modal-box{
          position:relative; width:100%; max-width:340px; background:linear-gradient(160deg,#161d42,#0d1330);
          border:1px solid rgba(232,200,116,0.35); border-radius:14px; padding:2rem 1.6rem 1.8rem;
          text-align:center; box-shadow:0 40px 80px -20px rgba(0,0,0,0.8), 0 0 40px rgba(232,200,116,0.12);
          animation:gdPopIn .35s cubic-bezier(.2,.9,.3,1.3);
        }
        @keyframes gdPopIn{from{opacity:0; transform:scale(0.85) translateY(10px);} to{opacity:1; transform:scale(1) translateY(0);}}
        .gd-modal-box p{font-size:1rem; line-height:1.7; color:#eef1ff; margin-bottom:1.6rem;}
        .gd-modal-btn-row{position:relative; display:flex; justify-content:center; gap:1rem; min-height:52px;}
        .gd-modal-yes, .gd-modal-no{
          font-family:'Outfit',sans-serif; font-weight:500; font-size:0.9rem; border-radius:999px;
          padding:0.65rem 1.6rem; cursor:pointer; border:none; transition:transform .2s ease;
        }
        .gd-modal-yes{background:var(--gold-soft); color:var(--navy-deep);}
        .gd-modal-yes:hover{transform:scale(1.05);}
        .gd-modal-no{
          background:transparent; color:var(--lav); border:1px solid rgba(205,212,240,0.4);
          position:relative;
        }
        .gd-modal-no.gd-fleeing{
          position:fixed; transition:top .25s ease, left .25s ease;
        }
        .gd-image-overlay{
          position:fixed; inset:0; z-index:60; background:rgba(6,8,20,0.85);
          display:flex; align-items:center; justify-content:center; padding:1.5rem;
          animation:gdFadeIn .3s ease;
        }
        .gd-image-box{
          position:relative; max-width:380px; width:100%; background:#fff8ec; border-radius:10px;
          padding:0.7rem 0.7rem 1.1rem; box-shadow:0 40px 90px -20px rgba(0,0,0,0.85);
          animation:gdPopIn .4s cubic-bezier(.2,.9,.3,1.3);
        }
        .gd-image-box img{width:100%; border-radius:4px; display:block;}
        .gd-image-caption{text-align:center; font-family:'Sacramento',cursive; font-size:1.3rem; color:#7a5a33; margin-top:0.6rem;}
        .gd-image-close{
          position:absolute; top:-14px; right:-14px; width:32px; height:32px; border-radius:50%;
          background:var(--gold-soft); border:none; color:var(--navy-deep); font-size:1rem; cursor:pointer;
      ...
      <footer className="gd-footer">
        ...
        <button type="button" className="gd-ask-btn" onClick={openAsk}>
          Click this
        </button>
      ...
      {/* ASK DIALOG */}
      {askOpen && (
        <div className="gd-modal-overlay" onClick={() => setAskOpen(false)}>
          <div className="gd-modal-box" onClick={(e) => e.stopPropagation()}>
            <p>let watch spiderman when i'm back from stung treng</p>
            <div className="gd-modal-btn-row">
              <button className="gd-modal-yes" onClick={acceptYes}>
                Yes
              </button>
              <button
                className={`gd-modal-no ${noPos ? "gd-fleeing" : ""}`}
                style={noPos || {}}
      ...
      {/* SURPRISE IMAGE POPUP */}
      {imageOpen && (
        <div className="gd-image-overlay" onClick={() => setImageOpen(false)}>
          <div className="gd-image-box" onClick={(e) => e.stopPropagation()}>
            <button
              className="gd-image-close"
            ...
            {/* <img
              src={`${import.meta.env.BASE_URL}photos/v1.png`}
              alt="Yay, it's a date!"
            /> */}
            {["v1.png"].map((file, idx) => (
              <div className="gd-polaroid" key={idx}>
            ...
            <div className="gd-image-caption">yay, it's a date 🕷️🤍</div>
          </div>
        </div>
      )}
    </div>
